;; uKanren using numbers as logic variables (the traditional way)

;; Supporting non-Kanren definitions

(defun (pair? x) (not (atom x)))

(defun (length l (acc 0))
  (if l (length (cdr l) (+ acc 1)) acc))

(defun (iota lim (i 0))
  (if (> lim i) (cons i (iota lim (+ i 1)))))
