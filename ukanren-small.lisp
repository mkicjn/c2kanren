...
; (Ported to the lisp-small implementation)
...
(defun (promise? x) (eq (car (car x)) 'lambda))
...
;; Finally, limiting the number of results we get without arithmetic support

; `take` - take a certain number `n` of elements from a stream `s` and construct a list out of them
; ...where n is given by a list of that length
(defun (take n s)
  (cond ((not n) ())
	((promise? s) (take n (force s)))
	((atom s) s)
	(t (cons (car s) (take (cdr n) (cdr s))))))
